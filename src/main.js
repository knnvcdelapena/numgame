...
import { getState, subscribe, startGame, goToSelect, goToLanding, playAgain, setGameMode, setDisplayStyle, setDigitCount, nextDigit, startRecall, setInput, submitRecall, getTimedDuration, initAuth, setUser, } from "./store";
...
function chunkSequence(seq, size) {
    const chunks = [];
...
function render(s) {
    ...
    if (s.phase === "recall" && currentPhase === "recall") {
        ...
        if (count)
            count.textContent = `${s.input.length} / ${s.sequence.length}`;
        if (btn)
            btn.disabled = s.input.length === 0;
        return;
    }
    ...
    if (s.phase === "select") {
        ...
        document
            ...
            btn.addEventListener("click", () => {
                ...
                const customInput = document.getElementById("custom-digit-input");
                if (customInput)
                    customInput.value = "";
            });
        });
        ...
        document
            ...
            .forEach((btn) => {
            btn.addEventListener("click", () => setDisplayStyle(btn.dataset.display));
        });
        document
